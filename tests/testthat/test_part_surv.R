...
surv_dist1 <- heemod::define_survival(
  distribution = "exp",
  rate = .003
)
surv_dist2 <- heemod::define_survival(
  distribution = "exp",
  rate = .002
)
surv_dist3 <- heemod::define_survival(
  distribution = "exp",
  rate = .005
)
surv_dist4 <- heemod::define_survival(
  distribution = "exp",
  rate = .004
)
...
suppressMessages(
  ps <- define_part_surv(
    pfs = heemod::join(surv_dist3, surv_dist4, at=365),
    os = heemod::join(surv_dist1, surv_dist2, at=365),
    cycle_length = c(365, 365)
  )
)
suppressMessages(
  ps1 <- define_part_surv(
    pfs = heemod::join(surv_dist3, surv_dist4, at=365) %>% heemod::apply_hr(0.8),
    os = heemod::join(surv_dist1, surv_dist2, at=365) %>% heemod::apply_hr(0.8),
    cycle_length = c(365, 365)
  )
)

sA <-  heemod::define_state(
  cost = 10000, ut = 1
)
sB <-  heemod::define_state(
  cost = 20000, ut = 1
)
sC <-  heemod::define_state(
  cost = 0, ut = 0
)

stratPS <- heemod::define_strategy(
  transition = ps,
  A = sA, B = sB, C = sC
)

stratPS1 <- heemod::define_strategy(
  transition = ps1,
  A = sA, B = sB, C = sC
)

suppressMessages(
  resPS <- heemod::run_model(
  Strat1 = stratPS,
  Strat2 = stratPS1,
  cycles = 10,
  cost = cost,
...
test_that(
  "part surv works", {
    ...
    surv_dist_1 <- heemod::define_survival(
      distribution = "exp", rate = 0.5)
    fit_cov <- flexsurv::flexsurvreg(
      formula = survival::Surv(rectime, censrec) ~ group,
      dist = "weibull", 
      data = flexsurv::bc)
    fitcov_medium <- heemod::set_covariates(fit_cov, group = "Medium")
    km_cov <- survival::survfit(
      formula = survival::Surv(rectime, censrec) ~ group,
      data = flexsurv::bc)
    km_medium <- heemod::set_covariates(km_cov, group = "Medium")
    
    suppressMessages({
      ps <- define_part_surv(
        pfs = surv_dist_1,
        os = km_medium %>%
          heemod::join(fitcov_medium, 
                  at = 730),
        cycle_length = c(1, 365)  # 1 for pfs, 365 for os
      )})
    
    sA <- heemod::define_state(cost = 10, ut = 1)
    sB <- heemod::define_state(cost = 20, ut = 0.5)
    sC <- heemod::define_state(cost = 0, ut = 0)
    
    stratPS <- heemod::define_strategy(transition = ps, A = sA, B = sB, C = sC)
    
    param <- heemod::define_parameters(
      p1 = heemod::compute_surv(
        surv_dist_1,
    ...
    tm <- heemod::define_transition(
      1 - p1, C, p2,
      0, C, p2,
      0, 0, 1)
    #> No named state -> generating names.
    
    stratTM <- heemod::define_strategy(
    ...
    expect_equal(
      heemod::get_counts(resPS)$count,
      heemod::get_counts(resTM)$count
    )
...
suppressMessages({
  ps <- define_part_surv(
    pfs = heemod::join(surv_dist3),
    os = heemod::join(surv_dist1),
...
test_that("we can run construct_part_surv_tib",
          {
            ...
            expect_equal(round(heemod::compute_surv(for_A[[1, "part_surv"]]$pfs, 1), 4), 0.0332)
            expect_equal(round(heemod::compute_surv(for_A[[1, "part_surv"]]$os, 1), 4), 0.0150)
